 ...
 *
 * Registers the `worktree` dictionaries and one chip on the new-session screen,
 * beside the agent-preset chip. The chip stages "start this session inside a
 * new worktree"; the Host route it calls creates the checkout, registers its
 * project, and starts the session there as one operation.
 *
...
export const inject = ['slots', 'locale', 'conversation', 'uiWorkspace']
